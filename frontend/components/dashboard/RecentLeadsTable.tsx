...
interface LeadItem {
  ...
  budget?: number; // fallback will be used if not populated
}
...
export default function RecentLeadsTable({ leads, onRefresh }: RecentLeadsTableProps) {
  ...
  const defaultLeads: LeadItem[] = [
    { id: 1001, name: "Siddharth Malhotra", phone: "+91 98765 43210", email: "siddharth@gmail.com", status: "NEW", source: "Website Form", assigned_to_id: 3, budget: 8500000 },
    { id: 1002, name: "Anjali Patil", phone: "+91 99887 76655", email: "anjali.patil@outlook.com", status: "CONTACTED", source: "Social Media", assigned_to_id: 3, budget: 12500000 },
    { id: 1003, name: "Kabir Mehra", phone: "+91 91234 56789", email: "kabir.mehra@yahoo.com", status: "VISIT_SCHEDULED", source: "Direct Walk-in", assigned_to_id: 4, budget: 35000000 },
    { id: 1004, name: "Neha Deshmukh", phone: "+91 90000 11111", email: "neha.d@deshmukh.in", status: "NEGOTIATION", source: "Property Portal", assigned_to_id: 5, budget: 18000000 },
    { id: 1005, name: "Aravind Sharma", phone: "+91 98888 77777", email: "aravind@sharmagroup.com", status: "CONVERTED", source: "Referral", assigned_to_id: 3, budget: 52000000 },
    { id: 1006, name: "Riya Kapoor", phone: "+91 97777 66666", email: "riya.k@gmail.com", status: "LOST", source: "Google Ads", assigned_to_id: 4, budget: 9500000 },
    { id: 1007, name: "Manish Pandey", phone: "+91 96666 55555", email: "manish.p@outlook.com", status: "NEW", source: "Website Form", assigned_to_id: 5, budget: 14000000 }
  ];

  const activeLeads = leads && leads.length > 0 ? leads : defaultLeads;

  // Formatting utility
  const formatCurrency = (val?: number) => {
    if (!val) return "₹1.2Cr"; // premium default
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            <input
              ...
              className="w-full sm:w-60 pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 text-xs focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-600/10 focus:border-blue-600 transition-all shadow-sm"
            />
          ...
          <select
            ...
            className="px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 text-xs font-medium focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-600/10 focus:border-blue-600 transition-all shadow-sm cursor-pointer"
          >
          ...
          <tbody className="divide-y divide-slate-100 text-sm">
            {paginatedLeads.map((lead) => (
              <tr key={lead.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4">
                  <p className="font-bold text-slate-900">{formatCurrency(lead.budget)}</p>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button 
                      ...
                      className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-700 text-xs font-bold hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm"
                    >
                      Manage
                    </button>
                  </div>
        ...
        <div className="p-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 bg-slate-50/30">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg border bg-white hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-semibold text-slate-600"
            >
              Previous
            </button>
